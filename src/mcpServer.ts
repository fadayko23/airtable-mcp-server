...
export class AirtableMCPServer implements IAirtableMCPServer {
	...
	private async handleListTools(): Promise<ListToolsResult> {
		...
		const result = {
			tools: [
				{ name: 'list_bases', description: 'List Airtable bases accessible with the API key', inputSchema: { type: 'object' as const, properties: {}, required: [] as string[], additionalProperties: false } },
				{ name: 'list_tables', description: 'List tables for a base', inputSchema: { type: 'object' as const, properties: { baseId: { type: 'string' as const } }, required: ['baseId'] as string[], additionalProperties: false } },
				{ name: 'describe_table', description: 'Describe a specific table (fields, views, primary field)', inputSchema: { type: 'object' as const, properties: { baseId: { type: 'string' as const }, tableId: { type: 'string' as const } }, required: ['baseId', 'tableId'] as string[], additionalProperties: false } },
				{ name: 'list_records', description: 'List records from a table (optionally limit and/or use a view)', inputSchema: { type: 'object' as const, properties: { baseId: { type: 'string' as const }, tableId: { type: 'string' as const }, maxRecords: { type: 'number' as const }, view: { type: 'string' as const }, filterByFormula: { type: 'string' as const } }, required: ['baseId', 'tableId'] as string[], additionalProperties: false } },
				{ name: 'create_record', description: 'Create a record in a table', inputSchema: { type: 'object' as const, properties: { baseId: { type: 'string' as const }, tableId: { type: 'string' as const }, fields: { type: 'object' as const, additionalProperties: true } }, required: ['baseId', 'tableId', 'fields'] as string[], additionalProperties: false } },
				{ name: 'update_records', description: 'Update one or more records in a table', inputSchema: { type: 'object' as const, properties: { baseId: { type: 'string' as const }, tableId: { type: 'string' as const }, records: { type: 'array' as const, items: { type: 'object' as const, properties: { id: { type: 'string' as const }, fields: { type: 'object' as const, additionalProperties: true } }, required: ['id', 'fields'] as string[], additionalProperties: false } } }, required: ['baseId', 'tableId', 'records'] as string[], additionalProperties: false } },
				{ name: 'delete_records', description: 'Delete one or more records from a table', inputSchema: { type: 'object' as const, properties: { baseId: { type: 'string' as const }, tableId: { type: 'string' as const }, recordIds: { type: 'array' as const, items: { type: 'string' as const } } }, required: ['baseId', 'tableId', 'recordIds'] as string[], additionalProperties: false } },
				{ name: 'search', description: 'Search for content and return a list of results with id, title, text, url', inputSchema: { type: 'object' as const, properties: { query: { type: 'string' as const } }, required: ['query'] as string[], additionalProperties: false } },
				{ name: 'fetch', description: 'Fetch a single result by id and return id, title, text, url, metadata', inputSchema: { type: 'object' as const, properties: { id: { type: 'string' as const } }, required: ['id'] as string[], additionalProperties: false } },
			],
		};
		...
		try {
			switch (request.params.name) {
				...
				case 'describe_table': {
					...
					const schema = await this.airtableService.getBaseSchema(baseId);
					const table = schema.tables.find((t) => t.id === tableId);
					if (!table) return formatToolResponse(`Table ${tableId} not found in base ${baseId}`, true);
